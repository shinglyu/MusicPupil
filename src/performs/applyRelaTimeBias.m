function outScores = applyRelaTimeBias(inScores, performParam);
   ...
   for scoreNo = 1: length(inScores)
      ...
      inScores{scoreNo}(:,6) = inScores{scoreNo}(:,6) + (performParam{scoreNo} .* (inScores{scoreNo}(end,6) + inScores{scoreNo}(end, 7)));
      inScores{scoreNo}(:,6) = max(inScores{scoreNo}(:,6), zeros(size(inScores{scoreNo}(:,6))));% no negatie onset time allowed
      %inScores{scoreNo}(:,6) = inScores{scoreNo}(:,6) + 60*performParam{scoreNo};
